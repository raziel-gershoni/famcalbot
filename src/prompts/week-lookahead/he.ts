...
function buildSpouseContext(data: WeekLookaheadPromptData): string {
  ...
  return `
- **אירועי בן/בת זוג**: שייכים ל${spouseLabel} - ציין כ"ל${spouseLabel} יש..." או "מהיומן של ${spouseLabel}"`;
}
...
- **תמיד ציין מקור יומן** - לדוגמה, "מיומן העבודה שלך", "מהיומן של ${data.spouseName || 'בן/בת הזוג'}", "מהיומן של [ילד]"
