...
3. **Другие события** - События детей и семейные события
   - События с тегом [Calendar: Child: Имя] принадлежат этому ребёнку
   - **КРИТИЧНО: Название события — это ПОЛНОЕ название учреждения/события — используйте его как есть.**
     "Тала Ган Гилад" — это одно название учреждения, а не "Ган" + имя ребёнка.
   - Пример: "Тала Ган Гилад середина года" [Child: Гилад] → У Гилада мероприятие середины года в Тала Ган Гилад
     (НЕ: "мероприятие середины года в саду" — отсутствует полное название учреждения)
   - **В порядке забора: используйте имя ребёнка, затем полное название учреждения в скобках**
`;
  }

  return `
3. **Другие события** - События детей и семейные события
   - **ВАЖНО: Не путайте названия учреждений/мест с именами детей**
   - Имена внутри названий событий (например: "Ган Гилад", "Тала", "Школа Рамон") — это названия учреждений, НЕ имена детей
   - **Определяйте имена детей только из названий календарей** (например: если календарь называется "Шира", ребёнка зовут Шира)
...
export function buildCalendarSummaryPrompt(data: SummaryPromptData): string {
  ...
  const kidsNamesLine = data.kidsNames && data.kidsNames.length > 0
    ? `- Дети: ${data.kidsNames.map(k => k.name).join(', ')}`
    : '';
...
- HH:MM - [Имя1] ([полное название учреждения1]), [Имя2] ([полное название учреждения2])
...
- HH:MM-HH:MM - [Имя] [Описание] ([полное название учреждения])
...
- HH:MM - [Имя] ([полное название учреждения])
- HH:MM - [Имя1] ([полное название учреждения1]), [Имя2] ([полное название учреждения2])
...
(Напоминание: Название события — это ПОЛНОЕ название учреждения/события. Например "Тала Ган Гилад" = одно название учреждения. Не разбивайте на части.)`
      : '**ДРУГИЕ СОБЫТИЯ (Дети и семья):**'
    : '**ДРУГИЕ СОБЫТИЯ:**';
