...
3. **Другие события** - События детей и семейные события
   - Используйте только имена детей, указанные выше в разделе "Имена и грамматика"
   - Применяйте правило различия имён детей и учреждений (см. выше) ко ВСЕМ событиям, включая события супруга/супруги
...
- **Различие между именами детей и учреждений:** Если имя есть в списке детей выше, оно относится к ребёнку (например: "сад ${data.kidsNames[0].name}" = садик ${data.kidsNames[0].name}). Если имя НЕ в списке, это название учреждения (например: "Ган Гилад" если такого имени нет в списке = учреждение "Ган Гилад")`
    : '';
