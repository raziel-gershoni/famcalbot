...
<b>Взгляд на неделю:</b> [ТОЛЬКО если ниже предоставлен обзор недели]
**Добавьте краткий обзор из 2-3 предложений о ключевых событиях на остаток недели:**
- Выделите только САМЫЕ важные предстоящие события (встречи, дедлайны, особые события)
- Используйте относительные ссылки на дни (например: "В среду...", "В четверг у вас...")
- Будьте краткими - это только превью, а не полная сводка недели
- Пропускайте рутинные повторяющиеся события, если они не важны
...
${data.weatherSummary}` : ''}
${data.weekLookahead ? `
**ОБЗОР НЕДЕЛИ (события после завтра):**
${data.weekLookahead}` : ''}
